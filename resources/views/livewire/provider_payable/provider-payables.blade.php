<div class="row sales layout-top-spacing">
            ...
            <div class="widget-heading">
                ...
                <div class="container">
                    <div class="row">
                        ...
                        @endcan
                        <div class="col-sm-3">
                            <a href="{{ url('provider_report/pdf' . '/' . $my_total . '/' . $search) }}" class="btn btn-dark btn-md" target="_blank">
                                Generar PDF
                            </a>
                        </div>
                        {{--<div class="col-sm-3">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped mt-1">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><h6 class="text-center">{{$payable->provider}}</h6></td>   
                                <td><h6 class="text-center">{{$payable->description}}</h6></td>                         
                                <td><h6 class="text-center">${{number_format($payable->amount,2)}}</h6></td>
                                <td class="text-center">
                                    <a wire:click.prevent="Details({{$payable->id}})" class="btn btn-dark mtmobile" title="Detalles">
                                        <i class="fas fa-list"></i>
                                    </a>
                                    @can('editar_registro')
                                    <a href="javascript:void(0)" wire:click.prevent="Edit({{$payable->id}})" class="btn btn-dark mtmobile" title="Editar" data-toggle="modal" data-target="#theModal2">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    @endcan
                                    @can('eliminar_registro')
                                    <a href="javascript:void(0)" onclick="Confirm('{{$payable->id}}')" class="btn btn-dark" title="Eliminar">
    ...
    @include('livewire.provider_payable.form')
    @include('livewire.provider_payable.form2')
    @include('livewire.provider_payable.detail')
    @include('livewire.provider_payable.data_import')
</div>
