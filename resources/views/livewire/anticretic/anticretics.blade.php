            ...
            <div class="widget-content">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped mt-1">
                        ...
                        <tbody>
                            @foreach ($anticretics as $anticretic)
                            <tr>
                                <td><h6 class="text-center">{{$anticretic->reference}}</h6></td>   
                                <td><h6 class="text-center">{{$anticretic->description}}</h6></td>                         
                                <td><h6 class="text-center">${{number_format($anticretic->amount,2)}}</h6></td>
                                <td class="text-center">
                                    <a wire:click.prevent="Details({{$anticretic->id}})" class="btn btn-dark mtmobile" title="Detalles">
                                        <i class="fas fa-list"></i>
                                    </a>
                                    @can('editar_registro')
                                    <a href="javascript:void(0)" wire:click.prevent="Edit({{$anticretic->id}})" class="btn btn-dark mtmobile" title="Editar" data-toggle="modal" data-target="#theModal2">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    @endcan
                                    @can('eliminar_registro')
                                    <a href="javascript:void(0)" onclick="Confirm('{{$anticretic->id}}')" class="btn btn-dark" title="Eliminar">
